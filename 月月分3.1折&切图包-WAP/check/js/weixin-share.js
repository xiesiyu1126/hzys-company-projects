...
$(function(){
	...
	var hostName = getMetaData("wx-env")=="test" ? "https://paymenttest.bankcomm.com" : "https://creditcardapp.bankcomm.com";
	//微信分享参数
	var targetlink = getMetaData("wx-link") || window.location.href; //点击分享后进入的页面url
	var title = getMetaData("wx-title") || $("title").text() || "交行信用卡中心";
	var desc = getMetaData("wx-desc") || window.location.href;
	var img = getMetaData("wx-img") || "";
	//调用jsonp获取config
	$.ajax({
		async:false,
		url: hostName+"/app/wechat/config.json?url="+link,
		dataType: "jsonp",
		jsonp: "callback",
